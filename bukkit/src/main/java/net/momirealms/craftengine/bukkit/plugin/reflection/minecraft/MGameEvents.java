...
public final class MGameEvents {
    ...
    private static Object getById(String path) {
        ...
        return RegistryUtils.getRegistryValue(BuiltInRegistriesProxy.GAME_EVENT, id);
    }

    private static Object getHolderById(String path) {
        Object id = IdentifierProxy.INSTANCE.newInstance("minecraft", path);
        if (VersionHelper.isOrAbove1_21_2()) {
            return RegistryProxy.INSTANCE.get$0(BuiltInRegistriesProxy.GAME_EVENT, id).orElseThrow();
        } else if (VersionHelper.isOrAbove1_20_5()) {
            return RegistryProxy.INSTANCE.getHolder$0(BuiltInRegistriesProxy.GAME_EVENT, id).orElseThrow();
        } else {
            return RegistryProxy.INSTANCE.getHolder$1(BuiltInRegistriesProxy.GAME_EVENT, ResourceKeyProxy.INSTANCE.create(BuiltInRegistriesProxy.GAME_EVENT, id)).orElseThrow();
        }
    }
}
