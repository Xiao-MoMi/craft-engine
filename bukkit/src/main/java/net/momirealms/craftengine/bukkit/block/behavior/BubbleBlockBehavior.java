...
public class BubbleBlockBehavior extends BukkitBlockBehavior {
  ...
  public BubbleBlockBehavior(CustomBlock customBlock, boolean drag, boolean direction, int limit) {
    ...
    if (drag) {
      this.blockState = LazyReference.lazyReference(
          () -> CraftEngine.instance().blockManager().createPackedBlockState("minecraft:bubble_column[drag=true]"));
    } else {
      this.blockState = LazyReference.lazyReference(
          () -> CraftEngine.instance().blockManager().createPackedBlockState("minecraft:bubble_column[drag=false]"));
    }
  }
  ...
  public void onRemove(Object thisBlock, Object[] args, Callable<Object> superMethod) throws Exception {
    ...
    if (direction) {
      ...
      for (int i = 0; i < limit; i++) {
        current = (BlockPos) LocationUtils.above(current);
        if (FastNMS.INSTANCE.method$BlockGetter$getBlockState(level, current) == MBlocks.BUBBLE_COLUMN) {
